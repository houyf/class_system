...
class Admin extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('username, password, last_logintime, level, real_name, email, class_id', 'required'),
			array('lock', 'numerical', 'integerOnly'=>true),
			array('username', 'length', 'max'=>100),
			array('password', 'length', 'max'=>40),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('admin_id, username, password, lock, real_name, last_logintime, email, send_email, level, class_id', 'safe', 'on'=>'search'),
		);
	}
